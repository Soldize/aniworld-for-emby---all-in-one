...
show_guide() {
    ...
    echo ""
    echo -e "  ${CYAN}API Server${NC} (Port $API_PORT)"
    echo "    Scrapt aniworld.to, speichert Anime-Katalog + Episoden in DB,"
    echo "    löst Stream-URLs auf (VOE via Headless Chromium). Das Herzstück."
    echo ""
    echo -e "  ${CYAN}Metadata Server${NC} (Port $META_PORT)"
    echo "    Holt Beschreibungen, Genres, Ratings, Status und Cover von"
    echo "    AniList/MAL. Cached alles lokal."
    echo ""
    echo -e "  ${CYAN}Proxy + Dashboard${NC} (Port $PROXY_PORT)"
    echo "    Der Proxy löst .strm Dateien zu echten Stream-URLs auf."
    echo "    Das Dashboard ist die Web-Oberfläche zum Steuern."
    echo ""
    echo -e "  ${CYAN}Sync Service${NC} (täglich 03:00)"
    ...
    echo ""
    echo "  Tabs:"
    echo -e "  - ${CYAN}📊 Dashboard${NC}  Service-Status, Sync-Buttons, Config Editor"
    echo -e "  - ${CYAN}🆕 Neu${NC}        Zuletzt hinzugefügte Anime/Episoden/Filme"
    echo -e "  - ${CYAN}🔍 Katalog${NC}    Anime-Suche, A-Z Navigation, Detail-Ansicht"
    echo -e "  - ${CYAN}⚙️ Config${NC}     Konfiguration direkt im Browser bearbeiten"
    echo -e "  - ${CYAN}⏰ Crons${NC}      Geplante Jobs verwalten"
    echo -e "  - ${CYAN}📋 Logs${NC}       Live-Logs aller Services (filterbar)"
    echo -e "  - ${CYAN}🔧 Settings${NC}   Passwort ändern, Backup/Restore"
    echo ""
    echo "  Scrape-Buttons:"
    echo -e "  - ${CYAN}🔄 Änderungen scrapen${NC}  Checkt laufende Serien auf neue Episoden"
    echo "    (überspringt abgeschlossene Anime automatisch)"
    echo -e "  - ${CYAN}📦 Batch Scrape${NC}        Holt Details für alle Anime (nur Erstinstall)"
    echo -e "  - ${CYAN}🔍 Einzeln${NC}             Einen Slug eingeben und nur den scrapen"
    echo ""
    echo -e "${BOLD}Ersteinrichtung nach Installation:${NC}"
    echo ""
    echo "  1. Dashboard öffnen: http://localhost:$PROXY_PORT/"
    echo ""
    echo "  2. Katalog wird automatisch gescraped beim API-Start."
    echo "     Dann '📦 Batch Scrape' klicken (holt Details, Cover, Staffeln)."
    echo "     Dauert ca. 2h bei der Erstinstallation."
    echo ""
    ...
    echo "     - Name: AniWorld"
    echo ""
    echo "  5. Emby Library Scan starten (oder [emby] Config setzen für Auto-Scan)."
    echo "     Fertig! 🎉"
    echo ""
    echo -e "${BOLD}Auto Library Scan (optional):${NC}"
    echo ""
    echo "  Damit Emby automatisch nach jedem Sync aktualisiert wird,"
    echo "  folgendes in $CONFIG_DIR/config.ini eintragen:"
    echo ""
    echo "    [emby]"
    echo "    url = http://localhost:8096"
    echo "    api_key = DEIN_API_KEY"
    echo ""
    echo "  API-Key findest du unter: Emby > Einstellungen > API-Schlüssel"
    echo ""
    echo -e "${BOLD}Backup / Restore:${NC}"
    echo ""
    echo "  Backup:  Installer Menü (7) oder Dashboard > Einstellungen > 💾 Backup"
    echo "  Restore: Installer Menü (8) oder Dashboard > Einstellungen > 📂 Restore"
    echo "  Enthält: DB (Anime-Katalog + Metadata), Config, Auth"
    echo "  Vor jedem Restore wird automatisch ein Backup der aktuellen Daten erstellt."
    echo ""
    ...
    echo ""
    echo "  VOE Streams laden nicht?"
    echo "    Playwright/Chromium muss installiert sein (passiert automatisch)."
    echo "    Test: journalctl -u aniworld-api | grep 'Playwright'"
    echo ""
